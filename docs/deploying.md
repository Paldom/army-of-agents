...
- Consumers run `npx skills add Paldom/army-of-agents` — the CLI
  ...
  users can also `gh skill install Paldom/army-of-agents <skill>
...
gh repo edit Paldom/army-of-agents --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/army-of-agents/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/army-of-agents --add-topic skills-sh
#    (gh < 2.90 has no `gh skill`; equivalent fallback:
#     gh release create v0.1.0 --generate-notes
#     gh repo edit Paldom/army-of-agents --add-topic agent-skills --add-topic skills-sh)
...
npx skills@latest add Paldom/army-of-agents --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
env -u DISABLE_TELEMETRY -u DO_NOT_TRACK npx skills@latest add Paldom/army-of-agents --skill '*' -a claude-code -y
...
curl -sIL https://skills.sh/Paldom/army-of-agents | head -1
```
...
(`https://skills.sh/b/Paldom/army-of-agents`) linking to the repo's
skills.sh page.
