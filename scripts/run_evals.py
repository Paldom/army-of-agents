...
def git_ignored(root: Path, paths: list[Path]) -> set[Path]:
    """The subset of `paths` git ignores. Empty when git is unavailable."""
    if not paths:
        return set()
    try:
        out = subprocess.run(
            ["git", "-C", str(root), "check-ignore", "--", *map(str, paths)],
            capture_output=True,
            text=True,
            check=False,
        ).stdout
...
def discover(root: Path) -> tuple[list[dict], list[str]]:
    ...
    invent competition the router never sees. Neither do gitignored ones: a
    third-party skill installed locally for authoring is not this repo's to
    score, and its evals are not this repo's to fix.
    """
    skills: list[dict] = []
    ...
    for base in (root / "skills", root / ".claude" / "skills"):
        ...
        ignored = git_ignored(root, children)
        for child in children:
            skill_md = child / "SKILL.md"
            if child in ignored or not skill_md.is_file():
                continue
